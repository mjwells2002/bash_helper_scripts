...
#usage remote-transcode-folder-1080p in-folder/ out-folder/
...
do
  ...
  then
    ...
    $remotessh -C ffmpeg -i $remote_tmp_folder$filename -s 1920x1080 -c:v libx264 -c:a aac -crf 21 -preset slow $remote_tmp_folder$filename-1080p-transcode.mp4
    scp $remote_host_string:$remote_tmp_folder$filename-1080p-transcode.mp4 $out_folder/
    $remotessh -C rm $remote_tmp_folder$filename
    $remotessh -C rm $remote_tmp_folder$filename-1080p-transcode.mp4
